...
void manipulacion_de_datos(float n_pixel_gauss);
...
int main(int argc, char **argv)
{
        ...
        char nombre[50] = "suave.png";
        char* nombre2 = nombre;
        float n_p = atof(argv[2]);
        leerpng(argv[1]);
        manipulacion_de_datos(n_p);
        escribirpng(nombre2);
...
void manipulacion_de_datos(float n_pixel_gauss)
{
      ...
      float **FMatrizR = (float **)malloc(alto * sizeof(float *));
       ...
       float **Gauss = (float **)malloc(alto * sizeof(float *));
       for (i=0; i<alto; i++)
       Gauss[i] = (float *)malloc(ancho * sizeof(float));
       gaussiana(Gauss,n_pixel_gauss);
       ...
       for (j=0; j<alto; j++)
       {
           for(i=0; i<ancho; i++)
           {
              CMatrizR[j][i] = FMatrizR[j][i]*Gauss[j][i];
              CMatrizG[j][i] = FMatrizG[j][i]*Gauss[j][i];
              CMatrizB[j][i] = FMatrizB[j][i]*Gauss[j][i];
           }
       }
